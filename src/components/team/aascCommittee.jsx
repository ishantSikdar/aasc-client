...
export default function AlumniCommittee({ isMobile }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const query = `
        *[_type == 'Faculty'] | order(_createdAt desc) {
          _id,
          name,
          aImage,
          Designation,
    ...
    <div className='my-20 flex flex-col gap-10 md:px-10'>
      <div className='text-[#853333] text-center flex flex-col items-center gap-2 md:mt-10'>
        <h2 className='text-3xl md:text-4xl lg:text-5xl font-bold uppercase'>
          Alumni Relations Executive Council
        </h2>
      ...
      {isLoading ? (
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-12 xl:gap-8 place-items-center'>
  {data.length > 0 ? (
    data.map((item) => (
      <div
        key={item._id}
      ...
      >
        ...
        <div className='text-center flex flex-col items-center gap-2'>
          <h3 className='text-[#853333] font-bold text-2xl'>{item.name}</h3>
          <div className='w-10 h-1 bg-[#853333]'></div>
          <p className='uppercase text-lg font-semibold text-gray-700'>{item.Designation}</p>
